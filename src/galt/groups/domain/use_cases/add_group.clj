...
(defn within-group-creation-limits?
  [{:keys [list-groups]} {:keys [founder-id]}]
  (>= max-group-per-member (count (list-groups {:founder-id founder-id}))))
...
(defn unique-name?
  [{:keys [list-groups]} group-creation]
  (empty? (list-groups {:name (:name group-creation)})))
