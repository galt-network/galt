...
(defn- add-post-links
  [deps post]
  (assoc post :post-link ((:link-for-route deps) :posts/by-id {:id (:id post)})))
...
(defn list-posts
  [{:keys [render layout asset-url] :as deps} req]
  (if (datastar-request? req)
      ...
      (fn [send!]
        (let [signals (get-signals req)
              ...
              [status result] (list-posts-use-case deps command)
               model (map (fn [p] (-> (add-post-links deps p) (update :author-avatar asset-url)))
                          result)]
    ...
    (let [limit 5
          ...
          [status result] (list-posts-use-case deps {:limit limit
                                                     ...
                                                     :end-time end-time})
          model {:new-post-href ((:link-for-route deps) :posts/new)
                 :posts (map (fn [p] (-> (add-post-links deps p) (update :author-avatar asset-url)))
                             result)
...
(defn new-post
  ...
  (let [model {:form-action (link-for-route :posts)
               :form-method "POST"
...
(defn show-post
  [{:keys [render post-repo layout link-for-route]} req]
  (let [member-id (get-in req [:session :member-id])
        ...
        comments-url (link-for-route :comments {:entity-id post-id :entity-type "posts"})
        edit-href (link-for-route :posts.by-id/edit {:id (:id post)})
        model {:post post
...
(defn edit-post
  ...
  (let [post-id (parse-uuid (get-in req [:path-params :id]))
        post (pr/get-post post-repo post-id)
        model {:post post
               :form-action (link-for-route :posts.by-id/edit {:id post-id})
               :form-method "PUT"}]
    {:status http-status/ok
     :body (-> model presentation.new-post/present layout render)}))

(defn update-post
  [{:keys [render update-post-use-case layout link-for-route]} req]
  (let [post-id (parse-uuid (get-in req [:path-params :id]))
    ...
    (m/match [status result]
           [:ok _] {:status http-status/see-other
                    :headers {"Location" (link-for-route :posts/by-id {:id post-id})}}
