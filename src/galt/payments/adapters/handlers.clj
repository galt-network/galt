...
(defn new-payment
  ...
  (let [payment-type (get-in req [:params :payment-type])
        ...
        invoice (membership-payment-use-case {:user-id user-id})
        sse-connection-id (:label invoice)
        datastar-action (d*-backend-action "/datastar-sse" :post {:connection-id sse-connection-id})
        model {:invoice-url (:bolt-11 invoice)
    ...
    (if (d*/datastar-request? req)
      (with-sse
        req
        (fn [send!] (send! :html (presentation.new-payment/payment-container model))))
...
(defn cln-webhook
  ...
  (let [events (req :body)
    ...
    (doseq [[type data] event-type-data]
      (case type
        "invoice_payment" (let [invoice (update-invoice-use-case {:label (:label data)})
